    ...
    int _n_time_ticks_data)
{
       ...
       i_block ++)
  {
    ...
    size_t current_block = i_block;
    size_t matched_block = _detector_properties_interface.same_plane_pair(plane, i_block);
...
void CorrelatedNoiseFilter::fix_correlated_noise_errors() {
  ...
  for (size_t i_plane = 0; i_plane < _correlatedNoiseWaveforms.size(); i_plane ++) {
    for (size_t i_block = 0; i_block < _correlatedNoiseWaveforms.at(i_plane).size(); i_block ++) {
