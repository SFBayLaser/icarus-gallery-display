...
class icarus(geometry):
    ...
    def __init__(self):
        # Try to get the values from the geometry file.  Configure for icarus
        # and then call the base class __init__
        super(icarus, self).__init__()
        larutil.LArUtilManager.Reconfigure(galleryfmwk.geo.kICARUS)
        self.configure()
        self._levels = [(20, -2), (-2, 20), (-1, 20)]
        # self._colorScheme =
        # self._time2Cm = 0.05515
        self._pedestals = [2000, 2000, 440]
        self._name = "ICARUS"
        self._logo = self._path + "/logos/icarus_logo_bw_transparent.png"
        self._logoRatio = 1.0
        self._haslogo = True
        self._logopos = [1250,10]
        self._logoscale = 0.1
